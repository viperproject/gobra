...
object Util{
	...
	//prints the values with the correct indentations for the commandline
		...
		input match {
			...
			case LitDeclaredEntry(name, value) => {
				value match {
					...
					case _ => s"$name(${prettyPrint(value,level)})"  
				} 
				
			}
			case v: WithSeq => {
				v.values.headOption match {
...
/** this is how we model the  values returned by the counterexamples...
  * the comments are a different mode of to string that might include more information.
  */
...
case class LitPermEntry(value: viper.silicon.state.terms.Rational) extends LitEntry {
	override def toString(): String = s"$value"
}
