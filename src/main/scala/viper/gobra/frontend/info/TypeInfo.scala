...
import viper.gobra.frontend.info.implementation.resolution.{AstPattern, MemberPath}
...
trait TypeInfo {
  ...
  def fieldLookup(t: Type, id: PIdnUse): Option[(StructMember, Vector[MemberPath])]

  def methodLookup(e: PExpression, id: PIdnUse): Option[(Method, Vector[MemberPath])]
  ...
  def resolve(d: PDot): Option[AstPattern with AstPattern.EntityLike]
  def resolve(i: PInvoke): Option[AstPattern]
  def isAssertion(e: PExpression): Boolean
  def isExpression(e: PExpression): Boolean
