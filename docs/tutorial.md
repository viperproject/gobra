...
- Structs containing floats compare field-wise with Go's float semantics. To prevent unsound reasoning, comparisons that the encoding could only model structurally (which deviates from Go's semantics for NaN and ±0.0) are rejected by the type checker: arrays containing floats cannot be compared, floats cannot occur in map key types, floats cannot be stored in interface values, and non-constant unbounded (ghost) integers cannot be converted to floats.
- The parser translates unary minus `-x` into `0 - x`, and Gobra encodes a subtraction from the literal zero as IEEE negation. Consequently, a user-written `0.0 - x` also negates: for x = +0.0 it yields -0.0 where Go yields +0.0, observable only through the sign of a zero result (e.g. by dividing by it).

Reasoning in the IEEE 754 theory can be expensive for the SMT solver. With the flag `--uninterpretedFloats`, Gobra falls back to encoding float operations as uninterpreted functions: much faster, but then nothing is known about float arithmetic (no literal values, no `NaN`, no rounding), and `==` on floats becomes an uninterpreted symmetric predicate (so `x == x` is not provable — `x` might be NaN — keeping the mode sound). The float support is tested with Z3 4.8.7, the version supported by Gobra's backend; newer Z3 versions have been observed to diverge on some floating-point queries (in particular when searching for counterexamples to invalid specifications).
