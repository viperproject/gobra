...
trait MemberResolution { this: TypeInfoImpl =>
  ...
  def tryPackageLookup(importedPkg: ImportT, id: PIdnUse): Option[(Entity, Vector[MemberPath])] = {
    def parseAndTypeCheck(importedPkg: ImportT): Either[Vector[VerifierError], ExternalTypeInfo] = {
      ...
      val res = for {
        pkgFiles <- PackageResolver.resolve(importPath, config.includeDirs).left.map(e => Vector(NotFoundError(e)))
        nonEmptyPkgFiles <- if (pkgFiles.isEmpty)
          Left(Vector(NotFoundError(s"No source files for import path '$importPath' found")))
          else Right(pkgFiles)
    ...
    def getTypeChecker(importedPkg: ImportT): Either[Messages, ExternalTypeInfo] = {
      def createImportError(errs: Vector[VerifierError]): Messages = {
        ...
        val importPath = importedPkg.decl.importPath
        val alternativeErr = context.getImportCycle(importPath) match {
          case Some(cycle) => message(importedPkg.decl, s"Import path '${importPath}' is part of this import cycle: ${cycle.mkString("[", ", ", "]")}")
          case _ => message(importedPkg.decl, s"Import path '${importPath}' contains errors")
        }
