...
case class MasterInterpreter(c: sil.Converter) extends GobraInterpreter{
	...
	def interpret(entry: sil.ExtractedModelEntry, info: Type): GobraModelEntry = {
		entry match{
			...
			case v: sil.VarEntry => interpret(c.extractVal(v), info)
			case d: sil.DomainValueEntry => if(d.getDomainName.contains("Embfn$$")){
											...
											info match {
												...
												case GhostSliceT(elem) => sliceInterpreter.interpret(d, SliceT(elem))
	...
	//deprecated type of a shared type
	...
	def interpret(entry: sil.DomainValueEntry, info: StructT): GobraModelEntry = {
		...
		if(doms.isDefined){
			...
			LitStructEntry(info, values)
				...
				case t: Throwable => return FaultEntry(s"${entry.domain} wrong domain for type: ${info.toString.replace("\n", ";")}")
			}
...
case class BoxInterpreter(c: sil.Converter) extends GobraDomainInterpreter[Type] {
	...
	def interpret(entry: sil.DomainValueEntry, info: Type): GobraModelEntry = {
		...
		if(unbox.isDefined && box.isDefined){
			val unboxed: sil.ExtractedModelEntry = try{Right(unbox.get.options.head._2) match{ //unboxing has some strange behaviour.
					case Right(x) => x
	...
	def interpret(entry: sil.DomainValueEntry, info: SliceT): GobraModelEntry = {
		...
		if(doms.isDefined){
			...
			if(funclength.isDefined && funcOffset.isDefined && funcSarray.isDefined){
				...
				/* deprecated, getting the entire arry instead of only the relevant idices
				...
				var curr: LitEntry = null;
				var map: Map[(Int, Int), LitEntry] = Map()
				// directly get the values from the indices
				def loc(v: BigInt) = {
...
case class PointerInterpreter(c: sil.Converter) extends sil.AbstractInterpreter[sil.RefEntry, Type, GobraModelEntry]{
	...
	def interpret(entry: sil.RefEntry, info: Type): GobraModelEntry = {
		...
		info match{
			case PointerT(elem ) => {
					...
					//
					val eField = extracted.fields.getOrElse(field, (sil.OtherEntry(s"$field: Field not found", s"help"), None))
					val perm = eField._2.map(LitPermEntry(_))
					val fieldval = if(extracted.fields.isEmpty) return  LitNilEntry()
						else if(eField._1.isInstanceOf[sil.OtherEntry]) extracted.fields.head._2._1 match {
												case r: sil.RefEntry => val value = interpret(r, info); return LitAdressedEntry(value.asInstanceOf[LitEntry], nameToInt(entry.name, filedname(entry, info)), perm) 
												case n: sil.NullRefEntry => return LitNilEntry() 
												case x => x 
						}
					InterpreterCache.addAddress(address, info);
					val value = eField._1 match {
			...
			case t => {
				// we are actually handling an shared value
	...
	def filedname(entry: sil.ExtractedModelEntry, i: Type): String = { //TODO: improve (maybe move to encoding of them)
		i match {
			...
			case x: StructT => entry match {
				case r: sil.RefEntry => s"val$$_ShStruct${x.fields.size}${if(x.fields.size == 0)""else "_"}${"Ref".repeat(x.fields.size)}"
				case _ => s"val$$_Tuple${x.fields.size}_${x.fields.map(x => filedname(null, x._2).replaceFirst("val$$_Tuple", ""))}"
			}
			case InterfaceT(decl, context) => "val$_Tuple2_RefTypes" 
			case DomainT(decl, context) => "TODO: domains"
...
case class StringInterpreter(c: sil.Converter) extends sil.ModelInterpreter[GobraModelEntry, Any] {
	...
	def interpret(entry: sil.ExtractedModelEntry, info: Any): GobraModelEntry = {
		...
		if(doms.isDefined){
			val functions: Seq[sil.ExtractedFunction] = doms.get.functions
			entry match{
				case e: sil.LitIntEntry => functions.find(x => x.fname.startsWith(prefix) && x.default == e) match {//regular string lit case
					case Some(f) => { val hex: String = f.fname.stripPrefix(prefix)
									val value = hexToChar(toArray(hex))
									LitStringEntry(value)

								}
					case _=>  functions.find(x => x.fname == Names.stringConact && x.image.contains(e)) match {//concatination case
...
case class InterfaceInterpreter(c: sil.Converter) extends GobraDomainInterpreter[InterfaceT]{
	...
	//extract type from the constructor function
	...
	// get the correct function from the pointers
	def getType(typ: sil.ExtractedModelEntry, typeDom: sil.DomainEntry): (Either[sil.ExtractedFunction, Seq[sil.ExtractedFunction]], Boolean) = {
		 //TODO: handle pointer to pointer
	...
	def isPointerViper(name: String): Boolean = { 
...
case class PredicateInterpreter(c: sil.Converter) extends GobraDomainInterpreter[PredT] {
	def interpret(entry: sil.DomainValueEntry, info: PredT) = {
		val preds = c.extractedHeap.entries.filter(x => x.isInstanceOf[sil.PredHeapEntry])
