...
Reasoning in the IEEE 754 theory can be expensive for the SMT solver. With the flag `--uninterpretedFloats`, Gobra falls back to encoding float operations as uninterpreted functions: much faster, but then nothing is known about float arithmetic (no literal values, no `NaN`, no rounding), and `==` on floats is structural. The float support is tested with Z3 4.8.7, the version supported by Gobra's backend; newer Z3 versions have been observed to diverge on some floating-point queries (in particular when searching for counterexamples to invalid specifications).
