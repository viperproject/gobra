...
- Structs containing floats compare field-wise with Go's float semantics, and maps with float keys are supported (keys compare structurally). Arrays of floats currently compare structurally — a NaN element does not make two arrays unequal, deviating from Go — and floats cannot be stored in interface values, since interfaces require types whose structural equality coincides with Go's equality. Similarly, map lookups treat NaN and ±0.0 keys structurally (in Go, NaN keys are unretrievable and +0.0/-0.0 are the same key).
