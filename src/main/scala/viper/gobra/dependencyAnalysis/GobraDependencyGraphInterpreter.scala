...
import viper.silicon.dependencyAnalysis.{DependencyGraphState, ReadOnlyDependencyGraph}


// TODO ake: implement or remove
class GobraDependencyGraphInterpreter[T <: DependencyGraphState](dependencyGraph: ReadOnlyDependencyGraph[T], typeInfo: TypeInfo, gobraErrors: List[VerifierError]) extends DependencyGraphInterpreter("gobra", dependencyGraph, List.empty, member=None) {
...
//      case PAssForRange(range, ass, spec, body) => PAssForRange(range, ass, pruneLoopSpec(spec), pruneBlock(body)) /*  */
//      case PShortForRange(range, shorts, bs, spec, body) => PShortForRange(range, shorts, bs, pruneLoopSpec(spec), pruneBlock(body)) // 
//
//      // switch-case, match
//      case PExprSwitchStmt(pre, exp, cases, dflt) => PExprSwitchStmt(pre, exp /*  */, cases.map(c => PExprSwitchCase(pruneExpressions(c.left), pruneBlock(c.body))), dflt.map(pruneBlock))
//      case PTypeSwitchStmt(pre, exp, binder, cases, dflt) => PTypeSwitchStmt(pre, exp /*  */, binder, cases.map(c => PTypeSwitchCase(c.left /*  */, pruneBlock(c.body))), dflt.map(pruneBlock))
//      // 
...
////      case PAssert(exp) => PAssert(pruneExpConjunctLevel(exp)) 
...
//  }
//
//	//  this functionality was moved to the PruningSupporter
...
////    println(newPkg)
////    throw new Exception("whatever") //  adjust return type
////  }
////
